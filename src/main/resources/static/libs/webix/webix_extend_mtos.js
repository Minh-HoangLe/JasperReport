...
mtos.ajax = {
    ...
    post_sync: function (application, service, operation, data, call) {
        ...
        var url = context+"/mtosapp/"+service+"/"+operation;
        ...
        .post(url, requestData, 
            {
                success: function (text, data, xhr) {
                    ...
                    if (!webix.isUndefined(result) && result.errorMessageProcessed) {
                        console.error('error: %o', result);
                        // return false;
                    }
                    call(text, data, xhr);
                }, 
                error: this._error_function
            }
        );
    },
    ...
    _headers: function(header) {
        if( header ) {
            return webix.extend(_ajax_headers, header);
        }

        return _ajax_headers;
    },

    _error_function: function(text, data, xhr) {
        // console.error('%o, %o, %o', text, data, xhr);
        
        // Authentication failed
        if( xhr.status === 401 ) {
            // title, _text, type
            commShowMessage.toastMsg('ERROR', 'Authentication failed', 'error');
            // window.location.reload();
        } else if ( xhr.status === 403 ) {
            commShowMessage.toastMsg('ERROR', 'Access forbidden', 'error');
        } else if ( xhr.status === 500 ) {
            if( text ) {
                var result = data.json();
                if( !result.success && result.message ) {
                    commShowMessage.toastMsg("ERROR", result.message, 'error');
                } else {
                    commShowMessage.toastMsg("ERROR", 'Server Internal Error', 'error');        
                }
            }
        } else {
            commShowMessage.toastMsg("ERROR", 'Server Internal Error', 'error');
    ...
    _calls: function(call) {
        var callback = {};

        if( call ) {
            if( typeof call.success === 'function') {
                callback.success = call.success;
            }

            if( typeof call.error === 'function') {
                callback.error = call.error;
            } else {
                // if error function not exist -> default 'error' function add
                callback.error = this._error_function;
            }

            // call is function -> 'success' function
    ...
    postSyncRequest: function(url, params, call, header) {
        webix.ajax().sync()
        .headers( this._headers(_json_headers) )
        .post(url, this._params(params), this._calls(call));
    },
    postAsyncRequest: function(url, params, call, header) {
        webix.ajax()
        .headers( this._headers(_json_headers) )
        .post(url, this._params(params), this._calls(call));
    },
    getSyncRequest: function(url, params, call, header) {
        webix.ajax().sync()
        .headers( this._headers(header) )
        .get(url, this._params(params), this._calls(call));
    },
    getAsyncRequest: function(url, params, call, header) {
        webix.ajax()
        .headers( this._headers(header) )
        .get(url, this._params(params), this._calls(call));
    }
...
webix.protoUI({
    ...
    $init: function (config) {
        ...
        this.data.attachEvent("onStoreLoad", function (driver, data) {
            ...
            owner._load_data = webix.toArray();
    ...
    deleteRow: function (data) {
        ...
        if (this.config.status) {
            ...
            data.forEach(function (obj) {
                owner.remove(obj.id);
                var loaded_index = mtos.hsheet.findIndexById(owner._load_data, obj.id);
                owner._load_data.removeAt(loaded_index);
            });
        ...
        if (this.config.status) {
            this.find(function (obj) {
                if (obj.DEL === 1) {
                    ...
                    var data = webix.copy(owner._load_data[loaded_index]);
                    data.DEL = 1;
                    data.ibflag = 'D';
                    result.push(data);
                }
    ...
    hideFilter: function () {
        ...
        this.eachColumn(function(id, col) {
            ...
            if (filter){
                ...
                else if (filter.stu) filter.indeterminate = true;
                else filter.value = "";					// html-based: select & text
            }
        });
        this.filterByAll(); //applies filters (necessary only in case of selectFilter)
